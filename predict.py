...
class Predictor(BasePredictor):
    def setup(self):
        """Initialize - API key validation moved to predict()"""
        # Configurar credenciales de Google Cloud para Imagen 3.0
        self.project_id = os.environ.get("GOOGLE_CLOUD_PROJECT", "luxscaler-project")
        self.location = "us-central1"
        print("Setup completado - esperando API keys en tiempo de ejecución")
    
    def _get_access_token(self):
        """Obtiene token de acceso para Imagen 3.0 Capability API"""
        try:
            if os.path.exists("/tmp/service-account-key.json"):
            ...
            print(f"Error obteniendo token de service account: {e}")
        return None
...
Responde SOLO con un objeto JSON válido, sin markdown ni explicaciones."""
    ...
    def enhance_with_soldier(self, tile: Image.Image, original_img: Image.Image, 
                            analysis: dict, scale_factor: int, is_full_image: bool = False) -> Image.Image:
...
Generate photorealistic detail, Phase One IQ4 camera quality, RAW-like depth."""
    ...
    def predict(
        ...
        # Validar API key en tiempo de ejecución
        if not gemini_api_key:
            gemini_api_key = os.environ.get("GEMINI_API_KEY", "")
        
        if not gemini_api_key:
            raise ValueError("gemini_api_key es requerido (pasar como input o variable de entorno)")
        ...
        if max_dimension <= 4096:
            ...
            final_img = self.enhance_with_soldier(upscaled, img, analysis, scale_factor, is_full_image=True)
            ...
            for ty in range(tiles_y):
                for tx in range(tiles_x):
                    ...
                    enhanced_tile = self.enhance_with_soldier(tile, img, analysis, scale_factor, is_full_image=False)
