...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        target_resolution: int = Input(description="Target longest side (px)", default=4096),
        ...
        print(f"Input: {input_img.width}x{input_img.height}")
        print(f"Target: {target_resolution}px (longest side)")
        
        # EL MEGA-PROMPT MAESTRO DE LUXIFIER
        master_prompt = f"""
**ROL DEL SISTEMA & OBJETIVO FINAL:**
NO ERES un editor de imágenes. ERES un motor de **reconstrucción generativa de ultra-alta fidelidad** (basado en protocolos tipo "Magnific/Deep Upscale"). Tu objetivo es tomar la imagen de entrada como un simple "mapa de guías" (identidad y composición) y **VOLVER A SOÑARLA** desde cero a una resolución masiva de {target_resolution}px, inventando detalles microscópicos que no existen en el archivo original pero que son biológica y materialmente obligatorios para el realismo.
...
* **PROHIBIDO SUAVIZAR.** Si un área de la imagen original (especialmente piel, tela o pelo) está borrosa, empastada o carece de definición, DEBES interpretar esto como "datos faltantes" que necesitan ser rellenados con generación sintética de alta frecuencia.
* **Inyección de Detalle Biológico (Piel):**
  * **NO GENERES "PIEL PERFECTA". GENERA TEJIDO VIVO.**
  * Debes sintetizar ("alucinar") una estructura compleja de poros individuales, variaciones en la capa córnea, micro-arrugas dinámicas alrededor de los ojos/boca y, crucialmente, **vello facial imperceptible (vellus hair)** en las mejillas y frente para dar realismo táctil.
...
* **Sensor Virtual:** Simula la captura con un respaldo digital de Formato Medio (Phase One IQ4, 150MP). Esto implica una profundidad de color y un rango dinámico extremos.
* **Lente y Foco:** Simula una lente "Prime" ultra-rápida (f/1.2). Aplica un enfoque crítico y "rabioso" (máxima acutancia) en los ojos y la textura de la piel. Todo lo que esté ligeramente fuera de ese plano focal debe caer en un bokeh cremoso y progresivo.

**FASE 4: REILUMINACIÓN VOLUMÉTRICA (ESCULTURA 3D)**
* Si la luz original es plana, destrúyela.
* Implementa una iluminación cinematográfica (ej. "Book Light" lateral suave pero direccional) que cree micro-sombras dentro de los nuevos poros y arrugas que has generado, esculpiendo el rostro con volumen tridimensional dramático (claroscuro).

**RESULTADO FINAL ESPERADO:**
Una imagen de {target_resolution}px que, al ser inspeccionada con lupa al 100%, no muestre artefactos de interpolación, sino una densidad abrumadora de información biológica y material sintética, indistinguible de una fotografía RAW de 100 megapíxeles.
        """
